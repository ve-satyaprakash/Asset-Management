...
export default function AssetStateForm({ record, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    setForm(record ? {
      ...
      requiresOwnership: record.requiresOwnership ?? false,
      requiresScan:      record.requiresScan      ?? false,
  ...
  async function handleSubmit(ev) {
    ...
    setSaving(true);
    try {
      if (record?.id) await updateAssetState(record.id, form);
