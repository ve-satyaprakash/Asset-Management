...
import { createSoftwareCategory, updateSoftwareCategory } from '../../services/softwareCategoryService';

const EMPTY = { name: '', description: '', isActive: true };

export default function SoftwareCategoryForm({ record, onSuccess, onCancel }) {
  ...
  async function handleSubmit(ev) {
    ...
    setSaving(true);
    try {
      if (record?.id) await updateSoftwareCategory(record.id, form);
      else            await createSoftwareCategory(form);
      onSuccess();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      <div className="px-5 py-4 space-y-4">
        ...
        <div>
          {lbl('Name', true)}
          <input name="name" value={form.name} onChange={handleChange} className={cls('name')} placeholder="e.g. Accounting" />
